import { loginHTML, editorHTML, paletteHTML } from '../lib/pages.mjs';
import { authenticated } from '../lib/auth.mjs';
export default async request => {
  if(request.method!=='GET')return new Response('Method not allowed',{status:405});
  if(['/editor/shop','/editor/shop/'].includes(new URL(request.url).pathname))return Response.redirect('https://admin.shopify.com/store/daisy-hatchet/products',302);
  const loggedIn=authenticated(request);
  const html=loggedIn?(new URL(request.url).pathname.replace(/\/$/,'')==='/editor/palette'?paletteHTML:editorHTML):loginHTML;
...
export const config={path:['/editor','/editor/','/editor/shop','/editor/shop/','/editor/gallery','/editor/gallery/','/editor/palette','/editor/palette/']};
