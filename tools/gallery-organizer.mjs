...
<script>
const grid=document.querySelector('.grid'),status=document.querySelector('.status'),save=document.querySelector('.save'),add=document.querySelector('.add');let items=[],photos=[],dragged='';
const esc=value=>String(value).replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[char]));
const markChanged=()=>{status.textContent='Unsaved changes';status.style.color='#f52f97'};
const render=()=>{grid.innerHTML='';items.forEach((item,index)=>{const card=document.createElement('article');card.className='card';card.draggable=true;card.dataset.id=item.id;const options=photos.map(file=>'<option value="'+esc(file)+'" '+(file===item.image?'selected':'')+'>'+esc(file.slice(0,8))+'</option>').join('');card.innerHTML='<img src="/images/'+encodeURIComponent(item.image)+'" alt=""><div class="fields"><label class="field">Arrangement name<input data-field="name" value="'+esc(item.name)+'"></label><label class="field">Price<input data-field="price" type="number" min="0" step="1" value="'+Number(item.price)+'"></label><label class="field image-field">Photo<select data-field="image">'+options+'</select></label></div><div class="card-footer"><label class="available"><input data-field="available" type="checkbox" '+(item.available?'checked':'')+'> Available</label><button class="remove" type="button">Remove</button></div>';grid.append(card)});};
...
grid.addEventListener('click',event=>{const button=event.target.closest('.remove');if(!button)return;const card=button.closest('.card');items=items.filter(item=>item.id!==card.dataset.id);render();markChanged()});
...
add.addEventListener('click',()=>{items.push({id:'arrangement-'+Date.now(),name:'New arrangement',price:40,image:photos[0],available:true});render();markChanged();window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'})});
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/shop') {
      ...
      const valid = Array.isArray(next) && next.length <= 100 && new Set(next.map((item) => item.id)).size === next.length && next.every((item) => item && typeof item.id === 'string' && item.id.length > 0 && typeof item.name === 'string' && item.name.trim().length > 0 && Number.isFinite(item.price) && item.price >= 0 && photos.includes(item.image) && typeof item.available === 'boolean');
      if (!valid) {
        response.writeHead(400, { 'content-type': 'application/json' });
