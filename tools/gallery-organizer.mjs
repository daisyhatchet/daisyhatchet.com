...
<header class="bar"><h1>Shop Manager</h1><div class="actions"><a class="preview" href="/">Manage gallery</a><a class="preview" href="http://127.0.0.1:4321/shop" target="_blank">Preview shop ↗</a><button class="save" type="button" disabled>Save shop</button><button class="publish save" type="button" disabled>Save and publish</button></div></header>
<div class="intro"><p>Edit arrangement names, prices, photos, and availability. Drag cards to change their order. Save keeps your edits local. Save and publish updates the live shop on daisyhatchet.com.</p><span class="status" aria-live="polite">Loading…</span></div>
...
const publish=document.querySelector('.publish');
const saveData=async(shouldPublish)=>{save.disabled=publish.disabled=true;grid.inert=true;add.disabled=true;status.textContent=shouldPublish?'Saving and publishing…':'Saving…';try{const response=await fetch('/api/shop'+(shouldPublish?'?publish=1':''),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(items)});const result=await response.json();if(!response.ok)throw new Error(result.error||'Could not save');status.textContent=result.message||'Saved — refresh the shop';status.style.color='#696a42';}catch(error){status.textContent=error.message;status.style.color='#b4235a';}finally{save.disabled=publish.disabled=false;grid.inert=false;add.disabled=false;}};
save.addEventListener('click',()=>saveData(false));publish.addEventListener('click',()=>saveData(true));
Promise.all([fetch('/api/shop').then(r=>r.json()),fetch('/api/order').then(r=>r.json())]).then(([saved,files])=>{items=saved;photos=files;render();status.textContent='Shop is saved';save.disabled=publish.disabled=false}).catch(()=>status.textContent='Could not load shop');
</script></body></html>`;
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/shop') {
      ...
      if (url.searchParams.get('publish') === '1') {
        const result = await publishEditorData(projectRoot, 'src/content/shop-items.json');
        response.writeHead(200, { 'content-type': 'application/json' });
        return response.end(JSON.stringify(result));
      }
      response.writeHead(200, { 'content-type': 'application/json' });
